...
currentAccount: any;
    ...
    loadAll() {
        this.branchService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<BranchMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/branch-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/branch-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BranchMySuffix) {
        return item.id;
    }
