...
@Component({
    selector: 'jhi-office-my-suffix',
    templateUrl: './office-my-suffix.component.html'
})
export class OfficeMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.officeService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<OfficeMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/office-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/office-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: OfficeMySuffix) {
        return item.id;
    }
